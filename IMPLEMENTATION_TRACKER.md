...
| B-002 | Review route | constrained | The named custom Hermes-Claude bridge is explicitly inactive and must not be activated without a failed native-adapter acceptance test and Matt’s decision. A Paperclip review-goal file write was blocked when the protected-file approval prompt timed out. | Direct restricted Claude review was used as a non-bridge fallback. A Paperclip-native bridge review still requires a fresh explicit approval path; record its issue ID and verdict before Phase 1. |
...
| R-003 | Pending Paperclip native adapter | Corrected Phase 0 blueprint bridge review | pending | pending | Protected review-goal write needs a fresh explicit approval path; then create, assign once, and read back verdict. |
